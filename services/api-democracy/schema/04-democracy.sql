...
CREATE TRIGGER democracy_delete BEFORE DELETE ON democracy FOR EACH ROW EXECUTE PROCEDURE date_deleted();
...
INSERT INTO democracy (id, name, description, population, rules, metas, parent_id) VALUES ('51a9a676-3b1e-47eb-845b-2784ccdd1d59','Test','test',10,'{"a":"b", "c":"d"}','{"rules":{ "add": { "approval_percent_minimum":60 }, "update":{ "approval_percent_minimum":60 }, "delete":{ "approval_percent_minimum":60 }}, "metas":{ "add": { "approval_percent_minimum":80 }, "update":{ "approval_percent_minimum":80 }, "delete":{ "approval_percent_minimum":80 }}}', (SELECT id FROM democracy WHERE parent_id IS NULL AND date_deleted IS NULL LIMIT 1));
