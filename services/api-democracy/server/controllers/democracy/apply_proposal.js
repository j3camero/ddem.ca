...
const democracy_create = async function({ democracy_id, democracy_name, democracy_description, democracy_population, democracy_rules, democracy_metas }) {
  const rows = await queryDB(this, 'INSERT INTO democracy(name, description, population, rules, metas) VALUES ($1, $2, $3, $4, $5) RETURNING id AS democracy_id, name AS democracy_name, description as democracy_description, population as democracy_population, rules as democracy_rules, metas as democracy_metas, date_created, date_updated;', [democracy_name, democracy_description, democracy_population, democracy_rules, democracy_metas])
...
const democracy_delete = async function(democracy_id) {
  const check = await queryDB(this, 'SELECT date_deleted FROM democracy WHERE id = $1;', [democracy_id])
  if(!check || check.length < 1 || !!check[0].date_deleted) {
		return 
	}
  const rows = await queryDB(this, 'DELETE FROM democracy WHERE id = $1 RETURNING id;', [democracy_id])
	return
}
...
const apply_proposal = async function(request, reply) {
	...
	let rows = await queryDB(this, 'SELECT * FROM democracy WHERE id = $1 AND date_deleted IS NULL', [democracy_id])
	...
	if(proposal.proposal_changes['add']) {
		for(let rule in proposal.proposal_changes['add']) {
			...
			if(democracy.id == root_democracy.id && proposal.proposal_target == 'rules.democracies') {
				democracy_add(proposal.proposal_changes['add'][rule])
			}
		}
	}
	if(proposal.proposal_changes['update']) {
		for(let rule in proposal.proposal_changes['update']) {
			...
			if(democracy.id == root_democracy.id && proposal.proposal_target == 'rules.democracies') {
				democracy_update(proposal.proposal_changes['update'][rule])
			}
		}
	}
