...
const proposal_read = async function(request, reply) {
	...
	const rows = await queryDB(this, 'SELECT id as proposal_id, democracy_id, name as proposal_name, description as proposal_description, target as proposal_target, changes as proposal_changes, date_created, date_updated FROM proposal WHERE id = $1;', [proposal_id])
	if(!rows || rows.length < 1) {
		request.log.error('Proposal/Read: Failure('+proposal_id+')')
		return replyError(reply, 400)
  }
	let proposal = rows[0]
	const counts = await queryDB(this, 'SELECT is_approved, COUNT(*) as cnt FROM ballot WHERE proposal_id = $1 GROUP BY is_approved ORDER BY is_approved', [proposal_id])
